...
cat "$dest_dir/train_src.cut.txt" > "$pair_dir/train_en"
...
for mode in train val test; do
    ...
    subword-nmt apply-bpe -c $EN_BPE_RULES < $2/${mode}_src.cut.txt > $1/${mode}_src.bpe

    # 对目标语言数据应用 BPE 规则，生成分词后的文件
    subword-nmt apply-bpe -c $ZH_BPE_RULES < $2/${mode}_trg.cut.txt > $1/${mode}_trg.bpe
